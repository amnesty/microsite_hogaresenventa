...
function validarFormFirma(f) {
    ...
    $("#"+id_form+" .error").css('visibility', 'hidden');
    error = 0;

    //VALIDAR DATOS PERSONALES
    if (f.nombre.value.search(/\S/) == -1 || f.nombre.value.length < 3) {

        $("#"+id_form+" .error.nombre").html("El nombre no puede estar vacío.");
        $("#"+id_form+" .form-group .error.nombre").css('visibility', 'visible');
    ...
    if(f.apellidos.value.search(/\S/) == -1 || f.apellidos.value.length < 3) {
        ...
        $("#"+id_form+" .error.apellidos").html("Los apellidos no pueden estar vacíos.");
        $("#"+id_form+" .error.apellidos").css('visibility', 'visible');
    ...
    if(f.email.value.search(/\S/) == -1) {
        ...
        $("#"+id_form+" .error.email").html("El email no puede estar vacío.");
        $("#"+id_form+" .error.email").css('visibility', 'visible');

        f.email.focus();
        error = 1;
    }else if (f.email.value.search(/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.[a-z]{2,}$/i) == -1) {

        $("#"+id_form+" .error.email").html("Email con formato incorrecto.");
        $("#"+id_form+" .error.email").css('visibility', 'visible');
    ...
    if(f.telefono.value.search(/\S/) == 0) {
        if(f.telefono.value.length != 9 || isNaN(f.telefono.value)) {
            $("#"+id_form+" .error.telefono").html("Teléfono con formato incorrecto.");
            $("#"+id_form+" .error.telefono").css('visibility', 'visible');
    ...
    if(f.pais.value == 0) {
        $("#"+id_form+" .error.pais").html("Selecciona el país de residencia.");
        $("#"+id_form+" .error.pais").css('visibility', 'visible');
